...
| `co_yiele`  | 返回一个数据，并且让协程暂停，然后等下一次机会恢复执行       |
...
| 类型                  | 作用         |
| --------------------- | ------------ |
| `std::suspend_never`  | 协程不挂起   |
| `std::suspend_always` | 协程需要挂起 |
...
| 函数                    | 作用                                          |
| ----------------------- | --------------------------------------------- |
| `get_return_object()`   | 获取协程返回值的对象                          |
| `return_value()`        | 给返回值对象赋值，调用co_return本质调用该函数 |
| `return_void()`         | 返回空结果                                    |
...
|            `address()`             |                    返回指向协程帧的指针。                    |
| `from_promise(promise_t& promise)` | 根据提供的 `promise_t` 对象，获取管理该 `promise_t` 的 `coroutine_handle`。 |
