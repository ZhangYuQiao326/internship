...
1. **异步编程**：在异步编程中，经常需要处理大量的 I/O 操作或等待事件完成，传统的线程模型往往会造成资源浪费和性能下降。协程可以让程序员更轻松地编写异步代码，避免了线程切换的开销，提高了程序的效率和性能。

2. **简化逻辑**：协程使得编写异步代码更类似于同步代码，因为它们可以在代码中使用顺序和同步的编程风格。这种方式更容易理解和维护，降低了开发的复杂性。

3. **资源管理**：协程的创建和销毁成本较低，不像线程那样消耗大量的系统资源。这使得可以创建大量的协程，每个协程可以代表一个较小的任务，从而更有效地利用系统资源。

4. **状态保存**：协程可以在挂起时保持其局部状态，而不需要像线程那样保存整个线程的状态。这意味着在恢复时，协程可以立即继续执行，而不需要重新计算状态。

总的来说，协程是一种轻量级、高效的并发编程工具，它们使得异步编程更加容易和高效，能够提高程序的性能和可维护性。
...
3. 创建promise对象
4. 判断协程执行前是否挂起
5. 协程执行结束，co_return返回结果，结束协程， co_yield返回结果后挂起
6. 协程结束后判断是否挂起(final_suspend)，否则直接结束，释放资源
