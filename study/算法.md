...
1. 回文子串：从前往后、从后往前读的字符都一样， abba
2. 使用二维dp表，存储 [i,j] 子串是否为回文串， 且使用二维表的上三角区域存储
3. 状态转移方程：判断前后字符一样的情况下，判断内部子串是否为回文串

| 步骤     | 含义                                                         |
| -------- | ------------------------------------------------------------ |
| 状态表示 | dp[ i] [ j ]: 字符串中，以j为起始，j为结尾的子串[i, j]是否是回文子串，填入true false |
| dp表     | 二维dp表，采用上三角表示是否为回文串，                       |
| 状态方程 | 如下                                                         |

![image-20240807224646251](https://cdn.jsdelivr.net/gh/ZhangYuQiao326/study_nodes_pictures@main/img/image-20240807224646251.png)
...
| 判断一个字串是否为回文串                                     | 双指针，值相等即为回文串             |
| [分割成回文字串的方案数](https://leetcode.cn/problems/palindrome-partitioning/description/) | 回溯                                 |
【回文子串个数】
    ...
    int countSubstrings(string s) {
        ...
        for(int i = n -1; i >=0 ; --i){ // 从下往上 
            for(int j = i; j < n; ++j){  // 从左往右
                // 判断前后元素相等，继而判断中间元素回文
                // 越界条件：s只有一个值、两个值且相等，直接为true
                // 两个以上，则判断[i+1,j-1]区间是否为回文子串
...
public:
    bool checkPartitioning(string s) {
        ...
        // 判断三个空间[0, i-1][i,j][j+1, n-1]是否为true
        // i、j都是从1开始，n-2结束
        for(int i = 1; i < n-1; i++){
            for(int j = i; j < n-1; j++){
...
* 子数组问题的状态表示均为：
  ...
  `dp[i] :以 i 位置为结尾的子数组中，的最大值、最小值、最大乘积、最小乘积`

* `dp[0] = 记录max值ret = nums[0]`, 全部用数组第一个值初始化

  ![image-20240808235805052](https://cdn.jsdelivr.net/gh/ZhangYuQiao326/study_nodes_pictures@main/img/image-20240808235805052.png)
...
![image-20240808230847075](https://cdn.jsdelivr.net/gh/ZhangYuQiao326/study_nodes_pictures@main/img/image-20240808230847075.png)

```cpp
class Solution {
public:
    int maxSubArray(vector<int>& nums) {
        int res = nums[0];  // 记录max
...
![image-20240808233153732](https://cdn.jsdelivr.net/gh/ZhangYuQiao326/study_nodes_pictures@main/img/image-20240808233153732.png)
...
public:
    int maxSubarraySumCircular(vector<int>& nums) {
        ...
        g[0] = nums[0]; //记录min
        int sum = nums[0];
        int maxj = j[0], ming = g[0];
        for(int i = 1; i < n ; ++i){
            // 最大和 
            j[i] = max(nums[i], j[i-1]+nums[i]);
            maxj = max(maxj, j[i]);
        ...
        return sum == ming ? maxj : max(maxj, sum - ming);
...
![image-20240808235839280](https://cdn.jsdelivr.net/gh/ZhangYuQiao326/study_nodes_pictures@main/img/image-20240808235839280.png)
...
[背包题解博客](https://leetcode.cn/problems/last-stone-weight-ii/solutions/805162/yi-pian-wen-zhang-chi-tou-bei-bao-wen-ti-5lfv/)

背包问题：1个背包，多个物品，选取物品装入背包，使得价值最大
...
完全背包：每个物品有无数个，可以重复选择

【模板】

![image-20240906114407761](https://cdn.jsdelivr.net/gh/ZhangYuQiao326/study_nodes_pictures@main/img/image-20240906114407761.png)

| 背包类型 | 返回类型     | dp[i]                                   |
| -------- | ------------ | --------------------------------------- |
| 01背包   | 最值问题     | `dp[i] = max(dp[i], dp[i - num] + val)` |
| 完全背包 | 是否存在问题 | `dp[i] = dp[i] || dp[i - num]`          |
|          | 排列组合问题 | `dp[i] += dp[i - num]`                  |

* 基本上都是 dp[i] 与 dp[i - num]之间的操作

【01背包】

* 外循正序循环物品容量大小，内 **倒叙** 循环背包容量大小

```cpp
int tar;
int nums[x,x,x];

vector<int> dp(tar + 1, 0);  // 根据容量创建dp表大小
for(auto num : nums)
{
    for(int i = tar; i >=   num; i--)  // 1 确保容量为i，大于物品的大小num
    {
        dp[i] = xx;
    }
}
...
| ------------------------------------------------------------ | ------------------ |
| 【增】合并有序数组                                           | 从后往前模拟       |
| 【删】[删除数组重复元素](https://leetcode.cn/problems/remove-duplicates-from-sorted-array/description/)、指定元素 | 朴素双指针         |
| 改                                                           |                    |
...
3 head用于遍历链表，判断是否合法，当head节点和尾节点的值不同时，将head加入到tail后
...
1. 创建`arr[128]`充当哈希表，记录某个字符是否存在，`hash[char] ++;`
1. `string son = str.sub(index, len)`获取切割子串，从index出发获取len位
