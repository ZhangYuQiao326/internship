...
```cpp
int removeRepeatVal(vector<int>& nums, int val)
{
	int len = nums.size();
	if (len == 1) return len;
...
int removeRepeatVal(vector<int>& nums, int val)
{
	...
	while(right < len)
	{
		...
		{
            // 重复元素不断递增
			while (nums[right] == nums[right - 1] && right < len)
			{
				right++;
			}
            // 找到不同的元素、数组后半部分的元素全部相同
            // 将left指向的重复元素回退
			left--;
		}
	}
...
| ------------------------------------------------------------ | ------------------ |
| 增                                                           |                    |
| [删除数组重复元素](https://leetcode.cn/problems/remove-duplicates-from-sorted-array/description/) | 快慢指针           |
| 改                                                           |                    |
| 查找数组元素/区间                                            | 朴素而非、升级二分 |
|                                                              |                    |
|                                                              |                    |
